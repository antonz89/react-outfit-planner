export default function clothingReducer(state, action){
    ...
    switch (action.type){
        ...
        case 'Casual':
            ...
            let casualTop = clothing.filter(e=>e.dressCode.includes('casual')&&e.type.includes("top"))

            let casualShoes = clothing.filter(e=>e.dressCode.includes('casual')&&e.type.includes("shoes"))
            
            let casualBottom = clothing.filter(e=>e.dressCode.includes('casual')&&e.type.includes("bottom"))
            
            return {

                a:casualTop[Math.floor(casualTop.length * Math.random())].imageUrl,
                b:casualShoes[Math.floor(casualShoes.length * Math.random())].imageUrl,
                c:casualBottom[Math.floor(casualBottom.length * Math.random())].imageUrl,
        
            }

        case 'Sport':
            let sportTop = clothing.filter(e=>e.dressCode.includes('sport')&&e.type.includes("top"))

            let sportShoes = clothing.filter(e=>e.dressCode.includes('sport')&&e.type.includes("shoes"))

            let sportBottom = clothing.filter(e=>e.dressCode.includes('sport')&&e.type.includes("bottom"))

            return {
                a:sportTop[Math.floor(sportTop.length * Math.random())].imageUrl,
                b:sportShoes[Math.floor(sportShoes.length * Math.random())].imageUrl,
                c:sportBottom[Math.floor(sportBottom.length * Math.random())].imageUrl,
            }
            
        case 'Formal':

            let formalTop = clothing.filter(e=>e.dressCode.includes('formal')&&e.type.includes("top"))

            let formalShoes = clothing.filter(e=>e.dressCode.includes('formal')&&e.type.includes("shoes"))

            let formalBottom = clothing.filter(e=>e.dressCode.includes('formal')&&e.type.includes("bottom"))

            return {
                a:formalTop[Math.floor(formalTop.length * Math.random())].imageUrl,
                b:formalShoes[Math.floor(formalShoes.length * Math.random())].imageUrl,
                c:formalBottom[Math.floor(formalBottom.length * Math.random())].imageUrl,
            }
